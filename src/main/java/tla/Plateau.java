...
public class Plateau {
    ...
    private boolean chronoEtat = false;
    private long temps;
...
//recherche si ce déplacement actionne un chrono
        Chrono chrono = niveau.CHRONOS.stream()
                .filter(t ->
                        t.getX() == joueur_x && t.getY() == joueur_y
                ).findFirst().orElse(null);
        if(chrono !=null && chronoEtat == false){
            Date date = new Date();
            temps = date.getTime() + chrono.getTemps() * 1000;
            chronoEtat = true;
        }
    ...
    private void apresDeplacement() {
        ...
        if (detecteCollisionFantome() || status()) {
            termineJeu(false);
            chronoEtat = false;
        } else {
